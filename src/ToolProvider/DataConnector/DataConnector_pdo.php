    ...
    public function loadToolConsumer($consumer)
    {
        ...
        if (!empty($consumer->getRecordId())) {
            $sql = 'SELECT consumer_pk, name, consumer_key256, consumer_key, secret, lti_version, ' .
                 ...
                 DataConnector::CONSUMER_TABLE_NAME . ' ' . 'WHERE consumer_pk = :id';
            ...
            $sql = 'SELECT consumer_pk, name, consumer_key256, consumer_key, secret, lti_version, ' .
                 ...
                 'profile, tool_proxy, settings, protected, enabled, ' .
                 'enable_from, enable_until, last_access, created, updated ' . "FROM {$this->dbTableNamePrefix}" .
                 DataConnector::CONSUMER_TABLE_NAME . ' ' . 'WHERE consumer_key256 = :key256';
            $query = $this->db->prepare($sql);
...
// Delete any outstanding share keys for resource links for contexts in this consumer
        $sql = 'DELETE sk ' . "FROM {$this->dbTableNamePrefix}" . DataConnector::RESOURCE_LINK_SHARE_KEY_TABLE_NAME .
             ...
             DataConnector::CONTEXT_TABLE_NAME . ' c ON rl.context_pk = c.context_pk ' . 'WHERE c.consumer_pk = :id';
        $query = $this->db->prepare($sql);
        $query->bindValue('id', $id, PDO::PARAM_INT);
        $query->execute();
...
// Update any resource links for which this consumer is acting as a primary resource link
        $sql = "UPDATE {$this->dbTableNamePrefix}" . DataConnector::RESOURCE_LINK_TABLE_NAME . ' prl ' .
             ...
             'SET prl.primary_resource_link_pk = NULL, prl.share_approved = NULL ' . 'WHERE rl.consumer_pk = :id';
        $query = $this->db->prepare($sql);
        $query->bindValue('id', $id, PDO::PARAM_INT);
        $query->execute();
...
// Update any resource links for contexts in which this consumer is acting as a primary resource link
        $sql = "UPDATE {$this->dbTableNamePrefix}" . DataConnector::RESOURCE_LINK_TABLE_NAME . ' prl ' .
             ...
             'SET prl.primary_resource_link_pk = NULL, prl.share_approved = NULL ' . 'WHERE c.consumer_pk = :id';
        $query = $this->db->prepare($sql);
        $query->bindValue('id', $id, PDO::PARAM_INT);
        $query->execute();
...
###
    public function getToolConsumers()
    {
        ...
        $sql = 'SELECT consumer_pk, name, consumer_key256, consumer_key, secret, lti_version, ' .
             'consumer_name, consumer_version, consumer_guid, ' . 'profile, tool_proxy, settings, protected, enabled, ' .
             'enable_from, enable_until, last_access, created, updated ' . "FROM {$this->dbTableNamePrefix}" .
    ...
    public function saveContext($context)
    {
        ...
        if (empty($id)) {
            ...
            $sql = "UPDATE {$this->dbTableNamePrefix}" . DataConnector::CONTEXT_TABLE_NAME . ' SET ' .
                 'lti_context_id = :ctx, settings = :settings, ' . 'updated = :updated ' .
...
// Update any resource links for which this consumer is acting as a primary resource link
        $sql = "UPDATE {$this->dbTableNamePrefix}" . DataConnector::RESOURCE_LINK_TABLE_NAME . ' prl ' .
             "INNER JOIN {$this->dbTableNamePrefix}" . DataConnector::RESOURCE_LINK_TABLE_NAME .
             ' rl ON prl.primary_resource_link_pk = rl.resource_link_pk ' .
             'SET prl.primary_resource_link_pk = null, prl.share_approved = null ' . 'WHERE rl.context_pk = :id';
        $query = $this->db->prepare($sql);
        $query->bindValue('id', $id, PDO::PARAM_INT);
        $query->execute();
...
// Delete resource link
    ...
    public function getSharesResourceLink($resourceLink)
    {
        ...
        $sql = 'SELECT consumer_pk, resource_link_pk, share_approved ' . "FROM {$this->dbTableNamePrefix}" .
             DataConnector::RESOURCE_LINK_TABLE_NAME . ' ' . 'WHERE (primary_resource_link_pk = :id) ' .
...
// Load the nonce
    ...
    public function saveConsumerNonce($nonce)
    {
        ...
        $sql = "INSERT INTO {$this->dbTableNamePrefix}" . DataConnector::NONCE_TABLE_NAME .
...
// Load share key
    ...
    public function saveResourceLinkShareKey($shareKey)
    {
        ...
        $sql = "INSERT INTO {$this->dbTableNamePrefix}" . DataConnector::RESOURCE_LINK_SHARE_KEY_TABLE_NAME . ' ' .
             '(share_key_id, resource_link_pk, auto_approve, expires) ' . 'VALUES (:id, :prlid, :approve, :expires)';
        $query = $this->db->prepare($sql);
        $query->bindValue('id', $id, PDO::PARAM_STR);
    ...
    public function saveUser($user)
    {
        ...
        if (is_null($user->created)) {
            ...
            $sql = "UPDATE {$this->dbTableNamePrefix}" . DataConnector::USER_RESULT_TABLE_NAME . ' ' .
                 'SET lti_result_sourcedid = :sourcedid, updated = :updated ' . 'WHERE (user_pk = :id)';
            $query = $this->db->prepare($sql);
            $query->bindValue('sourcedid', $user->ltiResultSourcedId, PDO::PARAM_STR);
            $query->bindValue('updated', $now, PDO::PARAM_STR);
